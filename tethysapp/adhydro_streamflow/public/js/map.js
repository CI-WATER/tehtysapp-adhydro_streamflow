...
var ERFP_MAP = (function() {
    ...
    var public_interface,                // Object returned by the module
        ...
        m_selected_feature,
        m_selected_adhydro_watershed,
        m_selected_adhydro_subbasin,
        m_selected_reach_id,
        m_selected_usgs_id,
        m_selected_nws_id,
        ...
        m_downloading_adhydro_hydrograph,
        m_downloading_usgs,
        m_downloading_nws,
    ...
    isThereDataToLoad = function(){
        return ((m_selected_adhydro_watershed != null && m_selected_adhydro_subbasin != null)
    ...
    isNotLoadingPastRequest = function() {
        return !m_downloading_long_term_select &&
            !m_downloading_usgs && !m_downloading_nws && !m_downloading_hydroserver &&
            !m_downloading_short_term_select && !m_downloading_adhydro_hydrograph;
    };
    ...
    getChartData = function() {
        if(!isNotLoadingPastRequest()) {
            ...
            //if there is a adhydro watershed & subbasin attribute
            if (m_selected_adhydro_watershed != null 
                && m_selected_adhydro_subbasin != null) {

                m_downloading_adhydro_hydrograph = true;
                jQuery.ajax({
                    type: "GET",
                    url: "adhydro-get-hydrograph",
                    dataType: "json",
                    data: {
                        watershed_name: m_selected_adhydro_watershed,
                        subbasin_name: m_selected_adhydro_subbasin,
                        reach_id: m_selected_reach_id,
                        date_string: m_adhydro_date_string,
                    },
                    success: function (data) {
                        if ("success" in data) {
                            ...
                            if ("adhydro" in data) {
                                var adhydro_series = {
                                    name: "ADHydro",
                                    data: convertTimeSeriesMetricToEnglish(data.adhydro),
                                    dashStyle: 'longdash',
                                    color: Highcharts.getOptions().colors[3]
                                };
                                var long_term_chart = $("#long-term-chart").highcharts();
                                long_term_chart.addSeries(adhydro_series);
                                $('.short-term-select').removeClass('hidden');
                                $('#long-term-chart').removeClass('hidden');
                            }
                        } else {
                            m_short_term_chart_data_ajax_load_failed = true;
                            appendErrorMessage("Error: " + data["error"], "adhydro_error", "message-error");
                            clearChartSelect2('short-term');
                        }
                    },
                    error: function (request, status, error) {
                        m_short_term_chart_data_ajax_load_failed = true;
                        appendErrorMessage("Error: " + error, "adhydro_error", "message-error");
                        clearChartSelect2('short-term');
                    },
                })
                .always(function () {
                    m_downloading_adhydro_hydrograph = false;
            ...
            //ADHydro Dates
            var adhydro_date_forecast_begin = new Date(8640000000000000);
            var adhydro_date_forecast_end = new Date(-8640000000000000);
            //get ADHydro forcast dates if available
            if(m_adhydro_date_string != null && typeof m_adhydro_date_string != "undefined" &&
                m_adhydro_date_string != "most_recent") {
                var adhydro_forecast_start_year = parseInt(m_adhydro_date_string.substring(0,4));
                var adhydro_forecast_start_month = parseInt(m_adhydro_date_string.substring(4,6));
                var adhydro_forecast_start_day = parseInt(m_adhydro_date_string.substring(6,8));
                var adhydro_forecast_start_hour = parseInt(m_adhydro_date_string.split("T")[1].substring(0,2));
                adhydro_date_forecast_begin = new Date(Date.UTC(adhydro_forecast_start_year, adhydro_forecast_start_month-1,
                                                         adhydro_forecast_start_day, adhydro_forecast_start_hour));
                adhydro_date_forecast_end = new Date(adhydro_date_forecast_begin.getTime()+15*60*60000);

                date_observed_end = adhydro_date_forecast_end;
                date_nws_end = adhydro_date_forecast_end;

            }

            var date_observed_start = new Date(Math.min.apply(null,[date_past, adhydro_date_forecast_begin]));
            var date_nws_start = new Date(Math.min.apply(null,[date_now, adhydro_date_forecast_begin]));
            ...
            if (m_selected_adhydro_watershed != null 
                ...
                .done(function (data) {
                    if ("success" in data && !m_short_term_chart_data_ajax_load_failed) {
                        ...
                        if (m_downloading_adhydro_hydrograph) {
                            $('.short-term-select').addClass('hidden');
                        }
                        //add on change event handler
                        $('#short-term-select').on('change.select2', function () {
                            m_adhydro_date_string = $(this).select2('data').id;
                            getChartData();
                        });
    ...
    loadHydrographFromFeature = function(selected_feature) {
        ...
        if(!isNotLoadingPastRequest()) {
            ...
            var adhydro_watershed_name = getCI(selected_feature, "watershed");
            var adhydro_subbasin_name = getCI(selected_feature, "subbasin");
            ...
            if(reach_id != null &&  
               adhydro_watershed_name != null &&
               adhydro_subbasin_name != null)
            {
                m_selected_feature = selected_feature;
                m_selected_reach_id = reach_id;
                m_selected_adhydro_watershed = adhydro_watershed_name;
                m_selected_adhydro_subbasin = adhydro_subbasin_name;
    ...
    *************************************************************************/
    ...
    $(function() {
        ...
        m_selected_adhydro_watershed = null;
        m_selected_adhydro_subbasin = null;
